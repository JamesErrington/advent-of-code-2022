...
func main() {
	file, err := os.Open("./05/question.txt")
	if err != nil {
		panic(err)
	}
	defer file.Close()
	...
	for _, instruction := range instructions {
		for i := 0; i < instruction.Value; i++ {
			element := stacks[instruction.From][len(stacks[instruction.From])-1]
			stacks[instruction.From] = stacks[instruction.From][:len(stacks[instruction.From])-1]
			stacks[instruction.To] = append(stacks[instruction.To], element)
		}
	}
